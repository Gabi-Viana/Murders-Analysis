...
function adicionarUsuario() {
    ...
    <label>Adicione um novo assassino:</label><br><br>
...
function adicionarAssassino2() {
    ...
    if((novoAssassino.nome=="") ||(novoAssassino.cpf=="")){
    
    } else{
    let repetir = 0
    for (let i = 0; i < informations.length; i++) {
       
        if (informations[i].nome == novoAssassino.nome && informations[i].cpfconfirm==novoAssassino.cpf) {
            repetir = 1
        }
    }
...
function changeInformations() {
    ...
    listaa.innerHTML = ` <label for="opcoes">Selecione o assassino que você deseja modificar as informações:<br></label>
        ...
        <input type="text"placeholder="Novo Nome" name="opções" id="nome-novo" class="adicionarCpf">
        <input type="text" placeholder="Novo CPF" class="adicionarCpf" id="cpf-novo"><br>
...
function deletar2() {
  ...
  function validar(){
    let dados={
    user: document.querySelector('#userValidation').value,
    cpf: document.querySelector('#cpfValidation').value
}

    let controle = 0

    for (let i = 0; i < informations.length; i++) {
        if (dados.user== informations[i].nome && dados.cpf == informations[i].cpfconfirm) {
            alert(`O nome ${dados.user}, CPF ${dados.cpf}, foi encontrado.`)
            i = informations.lenght
            controle = 1

        }
        else if (dados.user == informations[i].nome && dados.cpf != informations[i].cpfconfirm) {
            alert(`O nome ${dados.user}, CPF ${dados.cpf}, foi encontrado, porém o CPF é inválido.`)
            i = nome.lenght
            controle = 1
        }
    }
    ...
    if (controle == 0) {
        alert(`O nome ${dados.user}, CPF ${dados.cpf} não está na lista criminal.`)
    }
}
